...
mail_msg ='''
<p>是一个测试发邮箱的邮件啦：这里是内容
<p><a href="https://www.baidu.com">这里是连接
'''
# 如果仅添加正文可以用此公式：msg=MIMEText(mail_msg,'html','utf-8')#表示添加正文内容
msg.attach(MIMEText(mail_msg, 'html', 'utf-8'))# 添加正文
# 添加附件
att1 = MIMEText(open('E:\\pythonProject1\\report.html', 'rb').read(), 'base64', 'utf-8')
# 添加附件，由于定义了中文编码，所以文件可以带中文
att1["Content-Type"] ='application/octet-stream'  # 数据传输类型的定义
att1["Content-Disposition"] ='attachment;filename="test.html"'  # 定义文件在邮件中显示的文件名和后缀名，名字不可为中文
msg.attach(att1)# 将附件添加到邮件内容当中
# 配置调用邮件信息
msg['Subject'] ='测试邮件（测试报告）'  # 设置邮件主题
msg['From'] = user# 设置发件人
msg['To'] = to# 设置收件人
# 执行命令
